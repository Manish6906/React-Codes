...
<div className="" >
  <textarea className="form-control border border-1" onChange={xyz} value={text} style={{backgroundColor: props.mode==='light' ? 'white' : '#616161', color : props.mode==='light' ? 'black' : 'white' }} id="box1" rows="10"></textarea>
      ...
      <button className="btn btn-primary my-2" onClick={abc} >Convert to upperCase</button>
      <button className="btn btn-primary my-2  d-table" onClick={small}>Convert to LowerCase</button>
      <button className="btn btn-primary my-2  d-table" onClick={clearA}>Clear text in one click </button>
      <button className="btn btn-primary my-2  d-table" onClick={copyText}> Copy text in one click </button>
    </div>
    <div className="">
